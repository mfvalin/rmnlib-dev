...
int read_stream(int fd, void *ptr, int nbytes);
int write_stream(int fd, void *ptr, int n);
...
     if ((adr > wafile[ind].page[i].wa0) || 
         ...
         if ( nbytes < readbytes ) {
            ...
            fprintf(stderr," tried to get %d bytes, got %d\n",
                    readbytes,nbytes);
            fprintf(stderr,"WA_PAGE_SIZE=%d wa0=%d file_size=%ld\n",
...
*/
   ...
   if (adr > FGFDT[indf].file_size+1) {
      fprintf(stderr,"WA_PAGE_WRITE error: ind = %d, adr=%u > file_size=%ld\n",
...
//     nelm=write_stream(FGFDT[indf].fd,buf,nc);
    write_stream(FGFDT[indf].fd,buf,nc);
#if defined (DEBUG)  
...
//     nelm=read_stream(FGFDT[indf].fd,buf,nc);
    read_stream(FGFDT[indf].fd,buf,nc);
#if defined (DEBUG)
