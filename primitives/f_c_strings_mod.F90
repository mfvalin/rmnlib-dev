...
contains
  ...
  function allocate_string(this, n) result(ok)   ! create a new string of length at least n
    implicit none
  ...
  function clone_string(this, new) result(ok)  ! clone a string (copy by pointing)
    implicit none
  ...
  function duplicate_from_fstring(this, fstring) result(ok)   ! take a copy of a fortran character(len=*) string
    implicit none
    class(C_STRING), intent(INOUT) :: this
  ...
  subroutine delete_string(s)          ! deallocate string data (also used as finalize subroutine)
    implicit none
  ...
  subroutine deallocate_string(this)            ! deallocate a string
    implicit none
    class(C_STRING), intent(INOUT) :: this
    call delete_string(this)
  end subroutine deallocate_string

  function get_string_pointer(this) result(s)   ! string contents accessor
    implicit none
    class(C_STRING), intent(IN) :: this
  ...
  function is_string_owner(this) result(ok)     ! does the string own the storage space ?
    implicit none
    class(C_STRING), intent(IN) :: this
    logical :: ok
  ...
  function is_string_valid(this) result(ok)     ! is this string valid ?
    implicit none
    class(C_STRING), intent(IN) :: this
    logical :: ok
  ...
  subroutine strncpy_f2c(f_str, c_str, n)    ! copy a Fortran string into a NULL terminated character array of at most n characters
    implicit none
  ...
  subroutine strncpy_c2f(f_str, c_str, n)    ! copy a NULL terminated character array of up to n characters into a Fortran string 
    implicit none
