...
#include 'gmm.hf'
    ...
    type(gmm_attributes) :: attribs = gmm_attributes(0,0,0,0,0)
    ...
    integer *8 :: mykey
    integer :: reqid = 0
    integer i, ier, nbcles, status

    lxy(1)=lx
    lxy(2)=ly

    lyx=(/ ly, lx /)
    attribs%flags=GMM_FLAG_RSTR


   status = gmm_checkpoint_all(.true.)

  print *, 'Tentative de gmm_get'
  nullify(myarray)
  mon_meta = GMM_NULL_METADATA
...
!   call schacid2
   ...
   print *, 'gmm_keys-----------'
    ...
    nouveau_meta = GMM_NULL_METADATA
   ...
   ier = gmm_get('c3po_e', c3po_d, mon_meta)
   print *, c3po_a(1,1), c3po_b(1,1), c3po_c(1,1), c3po_d(1,1)
   status = gmm_checkpoint_all(.false.)
